...
После того как скрипт написан мы сохраняем файл и закрываем emacs. В терминале мы даём этому файлу право на выполнение и запускаем его (Рис. [-@fig:003]).

![Право на выполнение, запуск файла и проверка](image/3.png){ #fig:003 width=100% }

Теперь нам нужно доработать программу так, чтобы имелась возможность взаимодействия трёх и более процессов. Снова открываем emacs и наш файл sem.sh (Рис. [-@fig:004]).

![Доработка первого скрипта](image/4.png){ #fig:004 width=100% }

Сохраняем файл и проверяем его работу (Рис. [-@fig:005]).

![Сохранение и проверка](image/5.png){ #fig:005 width=100% }

В домашнем каталоге создаём файл man.sh, но уже для второго задания. Запускаем emacs (Рис. [-@fig:006]).

![Создание файла для второго задания](image/6.png){ #fig:006 width=100% }

Приступаем к реализации команды man с помощью командного файла. Изучим содержимое каталога /usr/share/man/man1. В нем находятся архивы текстовых файлов, содержащих справку по большинству установленных в системе программ и команд. Каждый архив можно открыть командой less сразу же просмотрев содержимое справки. Командный файл должен получать в виде аргумента командной строки название команды и в виде результата выдавать справку об этой команде или сообщение об отсутствии справки, если соответствующего файла нет в каталоге man1 (Рис. [-@fig:007]).

![Написание командного файла для второго задания](image/7.png)(image/7.png){ #fig:007 width=100% }

Сохраняем файл и даём в терминале право на выполнение. Запускаем файл man.sh для команды ls (Рис. [-@fig:008]) и (Рис. [-@fig:009]).
...
Снова в домашнем каталоге создаём файл. Запускаем emacs (Рис. [-@fig:010]). 

![Создание третьего файла](image/10.png){ #fig:010 width=100% }

После открытия файла random.sh напишем командный файл, генерирующий случайную последовательность букв латинского алфавита. Учтём, что $RANDOM выдаёт псевдослучайные числа в диапазоне от 0 до 32767. (Рис. [-@fig:011]).
	
![Написание третьего скрипта](image/11.png){ #fig:011 width=100% }

Сохраняем наш скрипт и даём право на выполнение. Запускаем файл для трёх разных чисел (Рис. [-@fig:012]). 
