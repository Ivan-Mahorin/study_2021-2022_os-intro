...
В домашнем каталоге создадим файлы common.h, server.c, client.c и Makefile, выполним проверку. Перейдём в emacs (Рис. [-@fig:001]).

![Создание файлов в домашнем каталоге](image/1.png){ #fig:001 width=100% }

В emacs откроем созданный файл common.h и приступим к переносу в него скрипта из файла, а также внесём в него дополнительные изменения (Рис. [-@fig:002]).

![Перенос скрипта для common.h](image/2.png){ #fig:002 width=100% }

После того как мы перенесли, изменили и сохранили скрипт для первого файла, открываем файл server.c и также переносим в него скрипт с изменениями, но уже для второго файла. Выполняем сохранение (Рис. [-@fig:003]).

![Перенос скрипта для server.c](image/3.png){ #fig:003 width=100% }

Теперь нам нужно перенести третий скрипт в файл client.c. После чего также выполняем сохранение (Рис. [-@fig:004]).

![Перенос скрипта для main.c](image/4.png){ #fig:004 width=100% }

Выполним перенос скрипта для последнего, четвёртого файла Makefile и закроем emacs (Рис. [-@fig:005]).

![Перенос скрипта для Makefile](image/5.png){ #fig:005 width=100% }

В терминале выполним команду make all. После чего в одном терминале запустим команду ./server, а в двух других ./client и проверим корректность выполнения (Рис. [-@fig:006]).
